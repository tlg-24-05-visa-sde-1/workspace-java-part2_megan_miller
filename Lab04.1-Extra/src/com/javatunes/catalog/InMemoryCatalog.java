...
public class InMemoryCatalog implements Catalog {
    ...
    public MusicItem findById(Long id) {
        ...
        for(MusicItem currentItem : catalogData) {
            if(currentItem.getId().equals(id)) {
                item = currentItem; // assign current item to item if the ID equals an ID in list
                break;
            }
        }
    ...
    */
    ...
    public Collection<MusicItem> findByKeyword(String keyword) {
        ...
        for(MusicItem currentItem : catalogData)
        {
            if(currentItem.getArtist().toLowerCase().contains(keyword.toLowerCase())||
                    currentItem.getTitle().toLowerCase().contains(keyword.toLowerCase()));{
    ...
    public int size() {
        ...
        // could return catalogData.size(), which returns an int with the catalog size
        for(MusicItem item : catalogData){
            count++;
        }
        return count;
